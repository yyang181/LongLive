...
"""Visualize a random LMDB sample with camera action overlay.

Reads one sample from a camera-aware LMDB (latents + intrinsics + poses),
decodes the Wan2.2 VAE latents back to pixel video, then composites a
Genie-3-style WASD + joystick action overlay driven by the stored camera
poses.  The result is written as a single .mp4 file.
...
def parse_args():
    ...
    p.add_argument("--lmdb_path", type=str,
                   ...
                   help="Path to LMDB directory containing data.mdb")
...
def main():
    ...
    # ---- Select sample ----
    count = get_lmdb_count(lmdb_path)
    ...
    viewmats, Ks = build_viewmats_and_Ks(sample["intrinsics"], sample["poses"])
    c2w = np.linalg.inv(viewmats)  # (F_lat, 4, 4) camera-to-world, frame-0 aligned

    # ---- Interpolate poses to match decoded video length ----
    # Poses are stored at latent resolution (stride-4 sampled); the VAE
    # decodes to (F_lat - 1) * 4 + 1 frames, so we must interpolate.
