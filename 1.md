...
`forward_speed=0.08`, `yaw_speed=pitch_speed=3°` per frame. `video_path` may
be absolute, relative to the current working directory, relative to the JSON
file, or relative to `VIDEO_DIR` below.
...
VIDEO_DIR=./dataset/LongLive/CameraSFT/raw/videos \
...
divisible by 16, otherwise the script exits early.
